...
class Directory final : public FsNode {
 public:
  ...
  size_t SmallestAtLeast(const size_t min_size, size_t smallest_so_far = std::numeric_limits<size_t>::max()) const {
    if (ByteSize() >= min_size && ByteSize() < smallest_so_far) {
      smallest_so_far = ByteSize();
    }
    for (const std::unique_ptr<Directory>& child : dirs_) {
      smallest_so_far = child->SmallestAtLeast(min_size, smallest_so_far);
    }
...
int main(int argc, char** argv) {
  ...
  const size_t free_space = 70000000 - root.ByteSize();
  const size_t needed_space = 30000000 - free_space;
  std::cout << root.SmallestAtLeast(needed_space) << "\n";
  return 0;
}
